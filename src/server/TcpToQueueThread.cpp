...
void TcpToQueueThread::run() {
  ...
  log_info("Handshake response already sent during client authorization check");

  log_info("End capability negotiate");
  ...
  while (SocketManager::isServerRunning()) {
    ...
    if (IsSocketReadable(socket_, 3000)) { // 100ms timeout
      log_info("Client -> Server (Receive TCP Data)");
      ...
      if (result == SOCKET_ERROR_TIMEOUT) {
        // Timeout, just continue the loop
          log_warnning("Timeout while receiving TCP data, continuing to next iteration");
        continue;
      } else if (result == SOCKET_ERROR_WOULD_BLOCK) {
          log_warnning("Would block while receiving TCP data, continuing to next iteration");
